...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
  ...
  <script src="./assets/js/material-dashboard.min.js?v=3.0.2"></script>
        ...
        <?php
            ...
            if(mysqli_num_rows($result) > 0){
                ...
                while($row = mysqli_fetch_assoc($result)) {
                    echo "<tr><td>" . $row["id"]. "</td><td>" . $row["room_no"] . "</td><td>"
                    . $row["room_type"]. "</td><td>" . $row["floor"]."</td><td>" . $row["beds"]. "</td>";
                }
            }
            else { echo "0 results"; }
    ...
    <form action="rooms2.php" method="post">
        <?php if (isset($_GET['error'])) { ?>
        <p class="error"><?php echo $_GET['error']; ?></p>
        <?php } ?>            
        <label for="room">Enter room number for further details</label>
        <input type="text" id="room" name="room" placeholder="101"><br>
